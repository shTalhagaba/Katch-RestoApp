...
const OrderSummary = ({ navigation, route, labs,...props }) => {
  ...
  const { startPolling, stopPolling } = useQuery(GET_USER_ORDER, {
  ...
  useEffect(() => {
    // if the order is in pending, accepted, ready, enRoute status, start polling
    if (order && order._id && pollingStatus.includes(order.orderStatus)) {
      startPolling(15000);
    } else {
      stopPolling();
    }
  ...
  }, [startPolling, stopPolling, order]);
