...
import { capitalizeFirstLetter, goToWhatsapp } from '../../components/Helpers';
...
const Landing = ({ navigation, ...props }) => {
  ...
  const landingContent = useQuery(GET_LANDING_CONTENT, {
    ...
    onCompleted: (query) => {
      ...
      setContact(query.getLandingContent?.orderSupport.whatsAppContact);
      setPopup(query.getLandingContent?.popup);
      scrollToTop();
      dispatch({
        type: 'landingContent',
  ...
  const [isItemsInCartVisible, setIsVisible] = useState(false);
  ...
  return (
    <>
      <DeepLinkListener navigation={navigation}>
        <View style={[styles.container, { paddingTop: statusBarHeight }]}>
          ...
          {/* {!!contact && (
            <Animated.View
              style={[
                ...
                { bottom: isItemsInCartVisible ? 70 : 10 },
              ]}>
              <TouchableOpacity onPress={() => goToWhatsapp(contact)}>
                <View style={styles.supportIMageWrapper}>
                  <Image style={styles.supportImage} source={WHATSAPP_ICON} />
                  <RText style={styles.supportText}>Support</RText>
                </View>
              </TouchableOpacity>
            </Animated.View>
          )} */}
          ...
          <View style={styles.mainContainer}>
            ...
            <ItemsInCart setIsVisible={setIsVisible} />
            <GiveAwayInfoModal
              isLoggedIn={authUser}
      ...
      <WalletGiveAway />
      {popup && <LandingModal popup={popup} />}
    </>
  );
};
