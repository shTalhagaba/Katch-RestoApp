...
import { convertOpeningTime } from '../../components/Helpers';
...
const getStoreHours = (storeHours) => {
  ...
  return [...Array(7)].reduce((acc, _, i) => {
    ...
    if (!isClosedForTheDay) {
      ...
      let isOpenNow = currentTime >= openTime && currentTime < closeTime;
            ...
            {todayStoreHours && (
              <View style={style.infoBox}>
                <RText style={style.infoText}>
                  {todayStoreHours.isOpenNow && info.isOpen
                    ? 'Open now '
                    : 'Closed '}
                </RText>
              </View>
  ...
  return (
    <View style={style.starContainer}>
      <View style={style.starInnerContainer}>
        ...
        <TextBasic style={style.reviewCountText}>
          {' '}
          ({info.restaurantReviewsCount} Reviews)
        </TextBasic>
      </View>
      <View style={style.starsTextContainer}>
